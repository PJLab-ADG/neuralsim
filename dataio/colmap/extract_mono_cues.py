...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', type=str, default='/data1/360_v2/bicycle')
    ...
    img_dir = os.path.join(data_dir, args.image_dirname)
    assert os.path.exists(img_dir)
    
    if args.task == 'depth':
        output_dir = os.path.join(args.data_dir, args.depth_dirname)
    elif args.task == 'normal':
        output_dir = os.path.join(args.data_dir, args.normals_dirname)
    else:
        raise RuntimeError(f"Invalid task={args.task}")
    os.makedirs(output_dir, exist_ok=True)
    
    img_names = list(sorted(os.listdir(img_dir)))
    for img_n in tqdm(img_names, f"processing {data_dir}"):
        fpath = os.path.join(img_dir, img_n)
