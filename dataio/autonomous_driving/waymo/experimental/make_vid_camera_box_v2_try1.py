"""
Performance: [60 mins @ downscale=2] 
Extremely slow, a sequence that could be processed in 3 minutes under the original v1 format now takes 50 minutes in this format.
"""
import os
import sys
...
def make_video(
    ...
    unique_ts = list(cam_image_w_box_w_asso['key.frame_timestamp_micros'].unique().compute())
    for find, ts in enumerate(tqdm(unique_ts)):

        im_per_cam = []
        cam_obs_this_frame_df = cam_image_w_box_w_asso.loc[cam_image_w_box_w_asso['key.frame_timestamp_micros']==ts]
        ...
        for camera_name in [v2._camera_image.CameraName.SIDE_LEFT, 
            ...
            # row = cam_image_w_box_w_asso.loc[
            #     (cam_image_w_box_w_asso['key.frame_timestamp_micros']==ts) &
            #     (cam_image_w_box_w_asso['key.camera_name']==camera_name)
            # ].compute()
            row = cam_obs_this_frame_df.loc[cam_obs_this_frame_df['key.camera_name']==camera_name].compute()
            row = row.squeeze()
            ...
            # # A for loop without any processing, takes about 12.6s / 5cam = 2.52s / iter
            # # Just the time spent on loop iteration totals to 2.5s * 5 cam * 199 frames = 40 mins
            # print(camera_image.velocity.linear_velocity.x)
