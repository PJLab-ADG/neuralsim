...
def get_frame_sampler(
    ...
    **kwargs, 
    ...
    You can use `dataset.get_scene_frame_idx` to retrieve the specific scene_idx and frame_idx.
    ...
    """
    scene_bank = dataset.scene_bank
    scene_lengths = [len(scene) for scene in scene_bank]
    num_frames = sum(scene_lengths)
    scene_weights = get_scene_weights(dataset, scene_sample_mode=scene_sample_mode)
    if frame_sample_mode == 'uniform':
        frame_weights = get_frame_weights_uniform(dataset, scene_weights, **kwargs)
    elif frame_sample_mode == 'weighted_by_speed':
        frame_weights = get_frame_weights_by_speed(dataset, scene_weights, **kwargs)
    elif frame_sample_mode == 'error_map':
        frame_weights = get_frame_weights_from_error_map(dataset, scene_weights, imp_samplers=imp_samplers, **kwargs)
    else:
        raise RuntimeError(f"Invalid frame_sample_mode={frame_sample_mode}")
    if ddp:
        return DistributedWeightedRandomSamplerSampler(
            num_samples=num_frames, mode='weighted_random', weights=frame_weights, 
            replacement=replacement, seed=seed, drop_last=drop_last)
    else:
        return WeightedRandomSampler(num_samples=num_frames, weights=frame_weights, replacement=replacement)
...
def get_frame_weights_uniform(
    ...
    assert len(scene_weights) == len(dataset.scene_bank), \
        "`scene_weights` should have the same length as scene_bank"
    total_weights = []
    for i, scene in enumerate(iter(dataset.scene_bank)):
...
def get_frame_weights_from_error_map(
    ...
    assert len(scene_weights) == len(dataset.scene_bank), \
        "`scene_weights` should have the same length as scene_bank"
    total_weights = []
    for i, (scene_id, scene) in enumerate(dataset.scene_bank.items()):
