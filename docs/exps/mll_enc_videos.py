...
from nr3d_lib.models.grid_encodings.permuto import GenerativeMLLConcat
...
m = GenerativeMLLConcat(
    n_latent_dim, 2, 3, 
    mll_cfg=ConfigDict(
        use_residual=True, 
        space_type='unbounded', aabb=[[0,0],[1,1]], 
        D=2, lattice_n_feats=2, lattice_n_levels=[8,16], output_activation='sigmoid', 
        lattice_cfg=ConfigDict(
            coarsest_res=16.0, finest_res=2000.0, 
            log2_hashmap_size=18, apply_random_shifts_per_level=True, 
            # param_init_cfg=ConfigDict(method='normal', std=1.0), 
            param_init_cfg=ConfigDict(method='uniform', bound=1.0e-4), 
        ), 
    ), 
...
#---- Opt 2: linspace, unlearnable 
#     NOTE: Tests show overfitting performance is far better than all zeros, loss convergence is very stable; but the continuous tweening effect is poor
latents_init = torch.linspace(-1, 1, len(imgs), dtype=torch.float, device=device).unsqueeze(-1).expand(len(imgs), n_latent_dim)

#---- Opt 3: Uniform distribution
...
latents.weight.requires_grad = False
...
with tqdm(range(num_iters)) as pbar:
    for _ in pbar:
        ...
        layer_zero.append(m.mll.lattice_layers[0].zero.item())
...
plt.plot(logging_loss, label='loss')
plt.plot(layer_zero, label='layer[0].zero')
plt.legend()
plt.show()
...
vid_pth = 'dev_test/test_permuto_vid_linspace.mp4'
imageio.mimwrite(vid_pth, vid)
print(f"Video saved to {vid_pth}")

vid0 = pred_im(latents_new, (800, 1200)).data.float().cpu().numpy()
vid0 = (vid0*255.).clip(0,255).astype(np.uint8)
vid0_pth = 'dev_test/test_permuto_vid0_linspace.mp4'
imageio.mimwrite(vid0_pth, vid0)
print(f"Video saved to {vid0_pth}")
