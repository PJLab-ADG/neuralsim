...
@brief  Reconstruct one or multiple multi-object scenes.
"""
import os
import sys
...
from nr3d_lib.utils import IDListedDict, check_to_torch, collate_nested_dict, import_str, backup_project, \
...
from app.resources import AssetBank, Scene, create_scene_bank, load_scene_bank
...
from dataio.data_loader import SceneDataLoader, ImageDataset, ImagePatchDataset, LidarDataset, PixelDataset, JointFramePixelDataset
...
class Trainer(nn.Module):
    def __init__(
        self, 
        config, 
        renderer: BufferComposeRenderer, 
        ...
        if 'latent' in loss_cfg:
            from app.loss import LatentLoss
            self.latent_loss = LatentLoss(**loss_cfg['latent'])
        
        self.deformation_loss = None
        if 'deformation' in loss_cfg:
            from app.loss import DeformationLoss
            self.deformation_loss = DeformationLoss(**loss_cfg['deformation'])

        if self.deformation_loss is not None:
            self.renderer._config_train.with_jacobian = True
    ...
    def forward(
        ...
        if self.debug_ret:
            ...
            for *k, v in nested_dict_items(ret):
                ...
                elif torch.isinf(v).any() and k[-1] != 't':
        ...
        if self.debug_grad:
            ...
            #----------------------------------------------------------------------------
            #-------------------   debug grad
            # NOTE: You can change object to foreground if something's wrong with foregrounds grad.
            self.debug_obj = scene.get_drawable_groups_by_class_name(self.debug_class_name)[0]
    ...
    def train_step_pixel(self, sample, ground_truth, it, logger: Logger=None):
        ...
        rendered_class_names = ret['rendered_class_names']
        #-----------------------------------------------
    ...
    def _validate_single_cam(
        ...
        rendered_per_class_in_scene = ret.get("rendered_per_class_in_scene", {})
        rendered_class_names = ret['rendered_class_names']
        #-----------------------------------------------
        ...
        log_imgs('', rendered)
        for class_name, cls_rendered in rendered_per_class_in_scene.items():
            log_imgs(f".{class_name}.in_scene", cls_rendered)
        ...
        if 'ray_intersections' in ret:
            add_log_img_entry("ray_intersections/obj_cnt", to_img(ret['ray_intersections']['obj_cnt'].unsqueeze(-1).float() / ret['ray_intersections']['obj_cnt'].max().clamp(1e-5)))
...
def main_function(args: ConfigDict):
    ...
    if args.ddp:
        # NOTE: When there are no intersecting rays with dynamic foregrounds / distant-view models, \
                ...
                with profile("Train on pixel dataloader"):
                    ...
                    with profile("Backward"):
                        ...
                        for n, o in asset_bank.named_optimzers(only_used=enable_grad_scaler):
                            scaler_pixel.unscale_(o) # TODO: Switch here based on whether clip-grad is needed, instead of unscale for all.
